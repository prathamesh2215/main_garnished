...
if((isset($obj->load_batch)) == "1" && isset($obj->load_batch))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		$sql_load_data  = " SELECT tb.*,tp.prod_name, ";
		...
		if($search_text != "")
		{
			$sql_load_data .= " and (prod_name like '%".$search_text."%'or batch_created like '%".$search_text."%' or batch_modified like '%".$search_text."%') ";	
		}
...
if((isset($obj->getProduct)) == "1" && isset($obj->getProduct))
{
	...
	{
		$data .='<option value="'.$row['id'].'">'.ucwords($row['prod_name']).'( '.ucwords($row['prod_unit_weight']).' )</option>';
	}
