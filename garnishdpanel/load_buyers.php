...
{
	$userid				   = mysqli_real_escape_string($db_con,$_POST['cust_id']);

	// quit($vendor_id);

	$data['cust_name']	= strtolower(mysqli_real_escape_string($db_con,$_POST['cust_name']));
	
	$data['cust_email']	= mysqli_real_escape_string($db_con,$_POST['cust_email']);
	$data['cust_mobile']	= mysqli_real_escape_string($db_con,$_POST['cust_mobile']);


	$pdata['pan_no']	       = mysqli_real_escape_string($db_con,$_POST['cust_pan']);
	$pdata['pan_modified']     = $datetime;
	$pdata['pan_modified_by']  = $uid;

	$gdata['gst_no']	       = mysqli_real_escape_string($db_con,$_POST['cust_gst']);
	...
	if($data['cust_name'] !="" && $data['cust_email']!="" && $data['cust_mobile']!="")
	{
		$data['cust_created'] = $datetime;
		$data['cust_created_by'] = $uid;
		update('tbl_customer',$data,array('cust_id'=>$userid));
...
if((isset($obj->load_customers_parts)) == "1" && isset($obj->load_customers_parts))
{
	...
	if($req_type != "")
	{
		...
		if($req_type=='edit' || $req_type=='view')
		{
			...
			$data .='<input type="hidden" name="cust_id" value="'.$row_get_data['cust_id'].'" />';


			$data .= '<div class="control-group">';
			$data .= '<label for="tasktitel" class="control-label">Name </label>';
			$data .= '<div class="controls">';
			$data .= $row_get_data['cust_name'];
			$data .= '</div>';	
			$data .= '</div>';	
	...
	if($page != "" && $per_page != "")	
	{
		...
		{		
			...
			while($row_load_data = mysqli_fetch_array($result_load_data))
			{
				...
				{				
						$customers_data .= '<td style="text-align:center">';
						$customers_data .= '<a  id="'.$row_load_data['cust_id'].'" class="btn-warning" href="ae_buyer.php?id='.$row_load_data['cust_id'].'&pag=Buyers">Edit</a></td>';				
				}
				$edit = checkFunctionalityRight("view_buyers.php",1);
				if($edit)
				{					
