                                ...
                                <div class="box box-color box-bordered">
                                    <div class="box-title">
                                        <h3>
                                            ...
                                            <button type="button" class="btn-info_1" style= "float:right" onClick="backToMain('div_add_customers','div_view_customers');" ><i class="icon-arrow-left"></i>&nbsp Back </button>                                       
                                    </div> <!-- header title-->
                                    <div class="box-content nopadding" >
...
<div class="modal fade" id="comments" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
	...
	<script type="text/javascript">
		...
		function addMoreVendor(cust_id,req_type)
		{
			...
			if ($('#frm_customers_add').valid())
			{
					...
					$.ajax({
						...
						success: function(response)
						{
							...
							if(data.Success == "Success") 
							{
								window.location.assign("ae_vendor.php?pag=Vendors&id="+data.resp[1]);
								loading_hide();
							} 
							else 
							{
			...
			if ($('#frm_edit_customers').valid())
			{
					...
					$.ajax({
						...
						success: function(response) 
						{
							...
							if(data.Success == "Success") 
							{								
								window.location.assign("view_vendors.php?pag=Vendors");
								loading_hide();									
							} 
							else 
							{
